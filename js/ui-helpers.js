// ══════════════════════════════════════════════════════════════
//  HTML ESCAPE — กันข้อความที่พนักงานพิมพ์เอง (ชื่อลูกค้า/สินค้า/หมายเหตุ ฯลฯ)
//  แอบมีแท็ก HTML ปนมาแล้วรันเป็นโค้ดจริงตอนแปะด้วย innerHTML (stored XSS)
//  ใช้ครอบเฉพาะค่าที่มาจากข้อมูล ไม่ใช่ตัว markup ที่เราเขียนเองในเทมเพลต
// ══════════════════════════════════════════════════════════════
...
// ค่าที่ต้องส่งเป็น "อาร์กิวเมนต์" ใน onclick — escapeHtml อย่างเดียวไม่พอ
// เพราะเบราว์เซอร์ถอด &#39; กลับเป็น ' ก่อน JS อ่าน SN ที่มี ' จึงหลุดออกจากสตริงแล้วสั่งงานต่อได้
// JSON.stringify ใส่เครื่องหมายคำพูดคู่ + escape ตัวข้างในให้เอง แล้วค่อย escapeHtml ทับอีกชั้น
// ใช้แบบ onclick="fn(${jsArg(v)})" — ไม่ต้องคร่อม ' เองอีก
function jsArg(v) {
...
function tab(name) {
  ...
  if (name === 'inbound') { renderInSession(); renderNameCodeNote(); setTimeout(() => document.getElementById('i-sn').focus(), 80); }
